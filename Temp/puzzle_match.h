...
int get_side_comparisson_points(struct PuzzleSide *s1, struct PuzzleSide *s2){
    ...
    int height_above1 = s1->heighest;
    int height_above2 = s2->heighest;
    int max_height_above = height_above1;
    if(height_above2>height_above1)
        max_height_above = height_above2;

    int height_under1 = s1->lowest;
    int height_under2 = s2->lowest;
    ...
    for(int i=0; i<n2; i++){
        ...
        bitmap[start_y+y][margin+x] = 3;
    }
    for(int i=shorter->positions[0].y+start_y; i>=0; i--)
        bitmap[i][margin+shorter->positions[0].x] = 3;
    for(int i=shorter->positions[n2-1].y+start_y; i>=0; i--)
